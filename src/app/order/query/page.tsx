...
export default async function OrderQueryPage({
  ...
  searchParams: Promise<{ orderNo?: string; queryToken?: string; error?: string }>;
}) {
  ...
  const { orderNo = '', queryToken = '', error = '' } = await searchParams;
      ...
      <section style={{ padding: 28, borderRadius: 24, border: '1px solid var(--border)', background: 'var(--card)' }}>
        ...
        <form action={submitQuery} style={{ display: 'grid', gap: 16, marginTop: 24 }}>
          <label style={{ display: 'grid', gap: 8 }}>
            <span>订单号</span>
            <input name="orderNo" required placeholder="例如：QX202604201122334455" defaultValue={orderNo} style={inputStyle} />
          </label>
          <label style={{ display: 'grid', gap: 8 }}>
            <span>查询密钥</span>
            <input name="queryToken" required placeholder="请输入下单后生成的查询密钥" defaultValue={queryToken} style={inputStyle} />
          </label>
...
const errorStyle: React.CSSProperties = { marginTop: 18, padding: 14, borderRadius: 14, border: '1px solid rgba(239,68,68,.24)', background: 'rgba(239,68,68,.12)', color: '#fca5a5' };
